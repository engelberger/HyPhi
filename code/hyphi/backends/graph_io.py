...
def graph_to_arrays(graph: nx.Graph, weight: str = "weight") -> GraphArrays:
    ...
    if we_arr.size and (we_arr <= 0.0).any():
        raise ValueError(
            "graph_to_arrays: all edge weights must be strictly positive (the 1d Forman "
            "kernel uses 1/sqrt(weight)). Found a non-positive weight; apply abs and remove "
            "zero-coupling edges first (signed CCORR values must be made positive)."
        )
